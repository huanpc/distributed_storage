...
//		System.getProperty("java.rmi.server.hostname", getIp());
		...
		Client clientRemote = new Client(tfServerIP.getText(), Constants.port, callHandler);
		System.setProperty("java.rmi.server.hostname", tfServerIP.getText());
	...
	private void btConnectActionPerformed(java.awt.event.ActionEvent evt)
		...
		if (tfServerIP.getText().equals("")) {
			...
			connectToServer();
			new MainGUI(defaultFile.getAbsolutePath(), client,
					server,tfServerIP.getText());
		} else {
			connectToServer();
			new MainGUI(
					chooser.getSelectedFile().getAbsolutePath(), client, server, tfServerIP.getText());
		}
